...
*/	
	...
	public void insertGrupeSampleData(SQLiteDatabase db) {
		...
		values.put(GrupaTable.NAME, "Pija�a");
	...
	//	SQLiteDatabase db = getWritableDatabase();
		...
		values.put(ProdListTable.NAME, "La�ko Pivo 3dl");
		//db.insert(TABLE, null, values);
		...
		// but for the inside only sample we'll just do it the old way:
		//Pija�a sample
		db.execSQL("insert into PROD_LIST (id, GRUPA_ID, name) values (2, 1, 'La�ko pivo 5dm')");
		...
		db.execSQL("insert into PROD_LIST (id, GRUPA_ID, name) values (2, 2, '�likrofi')");
